...
def creer_ecriture_groupee(date_str, ventes_jour, nb_ventes):
    ...
    total_caisse = 0.0  # ⭐ Argent reçu en caisse (montant_donne - rendu)
    ...
    for vente in ventes_jour:
        ...
        # ⭐ POINT CLÉ : Ce qui reste en caisse = montant_donne - rendu
        # ⭐ Si client donne 5000 et doit 3500, rendu = 1500, en caisse = 3500
        montant_effectif = montant_donne - rendu
        
        # ⭐ Caisse : montant effectif (ce qui reste en caisse)
        if montant_effectif > 0:
            total_caisse += montant_effectif
        ...
        # net_a_payer = total_actes + autres_frais - prise_en_charge
        # Ce que le client doit payer = net_a_payer
        # Ce qu'il a payé = montant_effectif (après rendu)
        reste_a_payer = net_a_payer - montant_effectif
        
        if reste_a_payer > 0.01:  # ⭐ Si le client doit encore de l'argent
        ...
        # ⭐ Si reste_a_payer < 0 : le client a trop payé
        # ⭐ MAIS comme il a reçu sa monnaie (rendu), ce n'est PAS un trop-perçu
        # ⭐ Le montant_effectif = net_a_payer exactement (après rendu)
        # ⭐ Donc reste_a_payer sera toujours >= 0 si rendu bien calculé
    ...
    # ⭐ 1. Débit : Caisse (MONTANT EFFECTIF = ce qui reste en caisse)
    if total_caisse > 0 and compte_caisse:
        ligne = LigneEcriture(
            ecriture_id=ecriture.id,
            compte_id=compte_caisse.id,
            debit=total_caisse,
            credit=0,
            libelle="Encaissements du jour"
        )
        db.session.add(ligne)
        total_debit += total_caisse
        lignes_ajoutees.append((f'Caisse (net encaissé)', total_caisse))
    ...
    if total_clients > 0 and compte_clients:
        ligne = LigneEcriture(
            ...
            debit=total_clients,
            credit=0,
            libelle="Créances clients (restes à payer)"
        )
        db.session.add(ligne)
    ...
    print(f"\n📊 Débit total: {total_debit:.2f}, Crédit total: {total_credit:.2f}")
    print(f"   Écart: {total_debit - total_credit:.2f}")
    
    # ⭐ 5. Vérification et équilibrage
    ...
    if abs(diff) > 0.01:
        ...
        if diff > 0 and compte_clients:
            # ⭐ Si crédit > débit, on augmente le débit (créances)
            ligne = LigneEcriture(
                ecriture_id=ecriture.id,
                compte_id=compte_clients.id,
                debit=diff,
                credit=0,
                libelle="Ajustement créances (écart)"
            )
            db.session.add(ligne)
            total_debit += diff
            lignes_ajoutees.append(('Ajustement clients (écart)', diff))
        elif diff < 0 and compte_caisse:
            # ⭐ Si débit > crédit, on augmente le crédit (remboursements)
            ligne = LigneEcriture(
                ecriture_id=ecriture.id,
                compte_id=compte_caisse.id,
                debit=0,
                credit=abs(diff),
                libelle="Ajustement caisse (écart)"
            )
            db.session.add(ligne)
            total_credit += abs(diff)
            lignes_ajoutees.append(('Ajustement caisse (écart)', abs(diff)))
    ...
    db.session.commit()
    print(f"\n✅ Écriture groupée créée (ID: {ecriture.id})")
    ...
    print(f"   {'✅ ÉQUILIBRÉ' if abs(total_debit - total_credit) < 0.01 else '❌ DÉSÉQUILIBRÉ'}")
    
    if dettes_details:
        print(f"\n   📋 {len(dettes_details)} client(s) avec créance")
        total_dettes = sum(d['montant'] for d in dettes_details)
        print(f"   💰 Total créances: {total_dettes:.2f} FCFA")
